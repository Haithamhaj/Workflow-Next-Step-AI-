...
function pass5CapabilitySpecBlocks(promptName: Pass5CapabilityPromptName, base: StructuredPromptSpec): StructuredPromptSpecBlock[] {
  ...
  const byPrompt: Record<Pass5CapabilityPromptName, [string, string][]> = {
    ...
    first_pass_extraction_prompt: [
      ...
      ["output_contract_or_schema", "Return strict JSON only matching FirstPassExtractionOutput, including structured SequenceMap, evidence anchors, unmappedContentItems, extractionDefects, and evidenceDisputes. All required array fields must be present. If there are no items for a required array, return an empty array []. Do not omit required arrays. Every extracted item must include evidenceAnchors[] and relatedItemIds[]. Every sequence link must include evidenceAnchors[]. Do not invent quotes or offsets."],
